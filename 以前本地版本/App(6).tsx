  ...
  const sendMessage = async (prompt: string, imageBlob?: Blob | File) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        console.log('[DEBUG] ========== 准备更新消息 ==========');
        console.log('[DEBUG] fullContent 长度:', fullContent?.length);
        console.log('[DEBUG] fullContent 前100字:', fullContent?.substring(0, 100));
        console.log('[DEBUG] imageBlob 存在:', !!imageBlob);
        console.log('[DEBUG] 当前 messages 数量:', messages.length);
        console.log('[DEBUG] =====================================');
        
        if (!imageBlob) {
          setMessages(prev => {
            const newMessages = [...prev, { role: 'assistant' as const, content: fullContent }];
            console.log('[DEBUG] 追问模式 - 新消息数量:', newMessages.length);
            return newMessages;
          });
        } else {
          const newMessages: Message[] = [userMessage, { role: 'assistant' as const, content: fullContent }];
          console.log('[DEBUG] 首次提问 - 新消息数量:', newMessages.length);
          console.log('[DEBUG] 用户消息:', userMessage);
          console.log('[DEBUG] AI消息长度:', fullContent.length);
